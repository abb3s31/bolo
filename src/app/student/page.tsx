...
// 🎓 بوابة تسجيل الدخول المخصصة لطلبة الجامعة (/student)
...
import { 
  loginStudentByEmailAndPassword, 
...
export default function StudentLoginPage() {
  ...
  const [emailInput, setEmailInput] = useState<string>(''); // ✉️ حقل البريد الأكاديمي للطالب
  const [passwordInput, setPasswordInput] = useState<string>(''); // 🔑 حقل كلمة المرور / الرمز السري للطالب
  const [showPassword, setShowPassword] = useState<boolean>(false); // 👁️ إظهار/إخفاء كلمة المرور
  ...
  // 🔄 فحص وجود جلسة نشطة مسبقاً والتحويل الفوري للوحة درجات الطالب
  useEffect(() => {
    const user = getCurrentSessionUser(); // 🔍 قراءة الجلسة المشفرة
    
    if (user) {
      setCurrentSessionUser(user); // 💾 حفظ المستخدم
      // 🎓 إذا كان طالباً يروح فوراً للداشبورد المخصصة إله
    ...
    // 🔀 إعادة التوجيه التلقائي للبوابة المركزية الموحدة مع تفعيل تبويب الطلاب
  ...
  const handleRememberMeToggle = (checked: boolean) => {
    ...
    if (!checked) {
      localStorage.removeItem('sadiq_remembered_student_email'); // 🧹 مسح فوري من التخزين
    } else if (emailInput.trim()) {
      localStorage.setItem('sadiq_remembered_student_email', emailInput.trim().toLowerCase());
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanEmail) {
      setErrorMessage('يرجى إدخال البريد الإلكتروني الأكاديمي للطالب.');
      setIsLoading(false);
      return;
    }

    if (!cleanPass) {
      setErrorMessage('يرجى إدخال كلمة المرور (الرمز السري) الخاصة بالطالب.');
      setIsLoading(false);
      return;
    }

    // 🔌 التحقق الحصري من حساب الطالب بالبريد وكلمة المرور من Supabase
    const result = await loginStudentByEmailAndPassword(cleanEmail, cleanPass);

    if (!result.user) {
      setErrorMessage(result.error || 'فشلت عملية التحقق من بيانات الطالب. يرجى مراجعة إدارة التسجيل.');
      setIsLoading(false);
      return;
    }
    ...
    if (rememberMe) {
      localStorage.setItem('sadiq_remembered_student_email', cleanEmail);
    } else {
      localStorage.removeItem('sadiq_remembered_student_email');
    }

    // 🎯 التوجيه المباشر للوحة درجات وسعي الطالب
    router.replace('/student/dashboard');
  };
        ...
        <div className="w-full max-w-sm bg-white border border-slate-200 rounded-3xl p-8 shadow-xl text-center space-y-5 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-center gap-2 text-slate-950 font-black text-sm">
              <Loader2 className="w-4 h-4 animate-spin text-indigo-800" />
              <span>جاري تدقيق الجلسة الأكاديمية للطالب...</span>
            </div>
            <p className="text-sm font-black text-slate-950 font-black">بوابة طلبة مسار بولونيا — فرع ميسان</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between font-sans selection:bg-indigo-600 selection:text-white" dir="rtl">
      ...
      <header className="w-full max-w-6xl mx-auto p-4 sm:p-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-black text-slate-950">فرع ميسان — بوابة الطلبة الرسمية</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">بوابة الأساتذة</span>
          </Link>
          <Link
            href="/admin"
            className="px-3.5 py-2 rounded-xl bg-white border border-slate-300 hover:bg-slate-100 text-slate-950 text-sm font-black transition flex items-center gap-1.5 shadow-2xs"
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
          ...
          <div className="text-center space-y-2">
            <div className="w-16 h-16 bg-indigo-50 border border-indigo-200 rounded-2xl flex items-center justify-center mx-auto text-indigo-900 shadow-xs">
              <GraduationCap className="w-8 h-8 text-indigo-800" />
            </div>
            <h2 className="text-xl sm:text-2xl font-black text-slate-950">تسجيل دخول الطلاب</h2>
            <p className="text-sm text-slate-950 font-black">
              استعراض الدرجات والسعي الفصلي، الجدول الأسبوعي، والموقف المالي
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* ✉️ حقل البريد الأكاديمي للطالب */}
            <div className="space-y-1.5">
              <label className="block text-sm font-black text-slate-950">
                البريد الإلكتروني الأكاديمي للطالب
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="student@sadiq.edu.iq"
                  dir="ltr"
                  className="w-full px-4 py-3.5 pl-11 text-sm sm:text-base font-black bg-slate-50 border border-slate-300 rounded-xl text-slate-950 placeholder:text-slate-700 focus:bg-white focus:border-indigo-600 focus:outline-none transition shadow-2xs text-left"
                />
            ...
            {/* 🔑 حقل كلمة المرور / الرمز السري للطالب */}
            <div className="space-y-1.5">
              <label className="block text-sm font-black text-slate-950">
                كلمة المرور / الرمز الأكاديمي
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder="أدخل الرمز السري للطالب..."
                  dir="ltr"
                  className="w-full px-4 py-3.5 pl-11 pr-11 text-sm sm:text-base font-black bg-slate-50 border border-slate-300 rounded-xl text-slate-950 placeholder:text-slate-700 focus:bg-white focus:border-indigo-600 focus:outline-none transition shadow-2xs text-left font-mono"
                />
                <Lock className="w-5 h-5 text-slate-950 absolute right-3.5 top-3.5 pointer-events-none" />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute left-3.5 top-3.5 text-slate-950 hover:text-slate-800 transition cursor-pointer"
            ...
            <div className="flex items-center justify-between text-sm pt-1">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  ...
                  className="w-4 h-4 text-indigo-700 rounded border-slate-300 focus:ring-indigo-500 cursor-pointer"
                />
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  <span>الدخول لبوابة الطالب</span>
                </>
              )}
            </button>
          </form>
          ...
          <div className="pt-3 border-t border-slate-200 text-center">
            <div className="flex items-center justify-center gap-1.5 text-sm font-black sm:text-sm text-slate-950 font-black">
              ...
              <span>دخول سريع ومؤمن بنظام التحقق المركزي لمسار بولونيا</span>
            </div>
          </div>
